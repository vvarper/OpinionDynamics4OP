...
def create_entry(folder_name, df_template, algorithm, local_od,
                 target, daily_factor, metric_columns,
                 month_columns, constrained):
    solution = pd.read_csv(f'{folder_name}/front.csv').iloc[0]

    solution_df = df_template.copy()
    solution_df['c_value'] = [algorithm, local_od, target,
                              daily_factor]
...
def main():
    ...
    parser.add_argument("-q", "--concern_threshold", type=float,
                        help="Concern threshold", default=0.7)
    ...
    results_dir = f'results/synth/summary_calibration_results/Concern{args.concern_threshold}/'
    ...
    df_template['Config'] = ['Algorithm', 'Local OD', 'Series', 'Days/Step']
    ...
    for target in targets:
        ...
        for local_od in local_ods.keys():
            ...
            folder_name = f'results/synth/{target}_history/Concern{args.concern_threshold}/'
        ...
        concern_all_results.to_csv(
            f'{results_dir}best_solutions_{target}.csv',
            index=False)

...
def main():
    ...
    output_dir = f'results/synth/{target_series}/Concern{args.concern_threshold}/'
    ...
    output_dir += (f'{args.local_od}_{args.daily_steps}_'
                   f'{args.mc}_{args.max_evaluations}_'
                   f'{args.pop_size}')
    ...
    crossover = {'generator': np.random.default_rng(
        seed=args.calibration_seed)}

    if not args.unconstrained:
        crossover['max_diff'] = 0.9
        crossover['min_diff'] = 0.1
        crossover['comparing_idx'] = problem.idx_paired_variables()
    ...
    calibration_info = {'mc': args.mc, 'daily_steps': args.daily_steps,
                        'local_od': args.local_od,
                        'target_series': target_series,
                        'fitness_function': 'Concern',
                        ...
                        'algorithm': 'GDE3', 'cr': args.cr, 'f': args.f,
                        'k': args.k,
